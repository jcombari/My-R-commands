...
#********************************************************
load(file="POBLACION_OBJETIVO_JULIO_AGOSTO.Rdata")
names(OBJETIVO_JULIO_AGOSTO)<-gsub("\\.super", "",names(OBJETIVO_JULIO_AGOSTO))
#COmprobamos que todos los creditos tengan más de 6 cuotas pendientes.

freq(OBJETIVO_JULIO_AGOSTO$CUOTA_PENDIENTE, useNA="ifany")
OBJETIVO_JULIO_AGOSTO$CUOTA_PENDIENTE<-as.numeric(OBJETIVO_JULIO_AGOSTO$PLAZO)-as.numeric(OBJETIVO_JULIO_AGOSTO$CUOTA_PAGA2)
freq(OBJETIVO_JULIO_AGOSTO$CUOTA_PENDIENTE, useNA="ifany")
OBJETIVO_JULIO_AGOSTO<-sqldf("select * from OBJETIVO_JULIO_AGOSTO where CUOTA_PENDIENTE>6")

#Cmprobamos que todos los creditos tengan más de 6 meses de haber sido desembolsado
freq(OBJETIVO_JULIO_AGOSTO$CUOTA_PENDIENTE, useNA="ifany")
library(zoo)
OBJETIVO_JULIO_AGOSTO$meses_desembolso<-as.numeric(round((as.Date(as.yearmon(as.character(OBJETIVO_JULIO_AGOSTO$fecha), "%Y%m"), frac = 1)-as.Date(OBJETIVO_JULIO_AGOSTO$FECDES) )/(365.25/12)))

freq(OBJETIVO_JULIO_AGOSTO$meses_desembolso, useNA="ifany")
OBJETIVO_JULIO_AGOSTO<-sqldf("select * from OBJETIVO_JULIO_AGOSTO where meses_desembolso>6")
freq(OBJETIVO_JULIO_AGOSTO$meses_desembolso, useNA="ifany")

#Variable calificacion
freq(OBJETIVO_JULIO_AGOSTO$CALIF_CART,useNA="ifany")
#Convertimos la califacion a numerico A=1 .... D=5
OBJETIVO_JULIO_AGOSTO$CALIF_CART<-as.numeric(as.factor(OBJETIVO_JULIO_AGOSTO$CALIF_CART))
freq(OBJETIVO_JULIO_AGOSTO$CALIF_CART,useNA="ifany")
CALIF_CART<-sqldf("select CEDULAENC, fecha, max(CALIF_CART) max_CALIF_CART from OBJETIVO_JULIO_AGOSTO group by CEDULAENC ,fecha ")
freq(as.factor(CALIF_CART$max_CALIF_CART), useNA="ifany")
#Creamos variable maduracion
OBJETIVO_JULIO_AGOSTO$MADURACION_SALDO<-(OBJETIVO_JULIO_AGOSTO$VLR_DESEMB-OBJETIVO_JULIO_AGOSTO$SAL_CAPITA)/OBJETIVO_JULIO_AGOSTO$VLR_DESEMB
percentiles <- function(x) {
  quantile(x, probs = setdiff(seq(0,1, by=0.05),seq(0,1)),na.rm = TRUE)
}

percentiles(OBJETIVO_JULIO_AGOSTO$MADURACION_SALDO)
OBJETIVO_JULIO_AGOSTO$MADURACION_CUOTA<-as.numeric(OBJETIVO_JULIO_AGOSTO$CUOTA_PAGA2)/as.numeric(OBJETIVO_JULIO_AGOSTO$PLAZO)
percentiles(OBJETIVO_JULIO_AGOSTO$MADURACION_CUOTA)

MADURACION<-sqldf("select CEDULAENC, fecha, max(CALIF_CART) max_CALIF_CART , count(OBLIGACIONENC) num_cred_libranza, SUM(SAL_CAPITA) SAL_CAPITA, sum(VLR_CUOTA) VLR_CUOTA, max(DIASMORA_I) DIASMORA_I, sum(VLR_MORA) VLR_MORA,  max(MADURACION_CUOTA) max_MADURACION_CUOTA, min(MADURACION_CUOTA) min_MADURACION_CUOTA, max(MADURACION_SALDO) max_MADURACION_SALDO, min(MADURACION_SALDO) min_MADURACION_SALDO  from OBJETIVO_JULIO_AGOSTO group by CEDULAENC ,fecha ")

#NUM_CREDITOS DE LIBRANZA

NUM_CREDITOS_LIBRANZA<-sqldf("select CEDULAENC, fecha, count(OBLIGACIONENC) num_cred_libranza from OBJETIVO_JULIO_AGOSTO group by CEDULAENC ,fecha ")
freq(NUM_CREDITOS_LIBRANZA$num_cred_libranza, useNA="ifany")

#PAGADURIAS Y OTROS

freq(OBJETIVO_JULIO_AGOSTO$COD_PAG, useNA="ifany")
#OBJETIVO_JULIO_AGOSTO$SAL_CAPITA
#Para cada cliente se queda la pagaduría asociada al saldo pendiente mas alto
CATEGORICAS<-sqldf("select CEDULAENC, fecha, MAX(SAL_CAPITA) MAX_SAL_CAPITA, COD_PAG from OBJETIVO_JULIO_AGOSTO group by CEDULAENC ,fecha")
...
# sqldf("select CEDULAENC, fecha, SAL_CAPITA,COD_PAG from OBJETIVO_JULIO_AGOSTO where fecha=201801 AND CEDULAENC='942947943762' ")
...
#Limpieza target 
freq(OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")
freq(OBJETIVO_JULIO_AGOSTO$MOD_DEF, useNA="ifany")
table(OBJETIVO_JULIO_AGOSTO$MOD_DEF,OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")

OBJETIVO_JULIO_AGOSTO$TARGET<-ifelse(OBJETIVO_JULIO_AGOSTO$TARGET==1 & OBJETIVO_JULIO_AGOSTO$MOD_DEF=="FIDELIZACION", 0 , OBJETIVO_JULIO_AGOSTO$TARGET)

freq(OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")
table(OBJETIVO_JULIO_AGOSTO$MOD_DEF, OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")
OBJETIVO_JULIO_AGOSTO$TARGET[is.na(OBJETIVO_JULIO_AGOSTO$TARGET)] <- 1
table(OBJETIVO_JULIO_AGOSTO$MOD_DEF, OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")

#Creamos MARCA DE FIDELIZACION DENTRO DE LA VENTANA

OBJETIVO_JULIO_AGOSTO$marca_fid_ventana<-ifelse(OBJETIVO_JULIO_AGOSTO$MOD_DEF=="FIDELIZACION" & as.numeric(OBJETIVO_JULIO_AGOSTO$FECDES_NF)<=as.numeric(OBJETIVO_JULIO_AGOSTO$lista_fin) ,1,OBJETIVO_JULIO_AGOSTO$TARGET)
freq(OBJETIVO_JULIO_AGOSTO$marca_fid_ventana)
OBJETIVO_JULIO_AGOSTO$marca_fid_ventana[is.na(OBJETIVO_JULIO_AGOSTO$marca_fid_ventana)] <- 0
freq(OBJETIVO_JULIO_AGOSTO$marca_fid_ventana)

OBJETIVO_JULIO_AGOSTO$TARGET2<-ifelse(OBJETIVO_JULIO_AGOSTO$MOD_DEF=="FIDELIZACION" & OBJETIVO_JULIO_AGOSTO$marca_fid_ventana==1,1,OBJETIVO_JULIO_AGOSTO$TARGET)

table(OBJETIVO_JULIO_AGOSTO$MOD_DEF, OBJETIVO_JULIO_AGOSTO$TARGET2, useNA="ifany")

aux<-freq(OBJETIVO_JULIO_AGOSTO$TARGET, useNA="ifany")
tasa_churn<-(aux[2]/aux[1])*100

aux2<-freq(OBJETIVO_JULIO_AGOSTO$TARGET2, useNA="ifany")
tasa_churn_fid<-(aux2[2]/aux2[1])*100

cbind(tasa_churn, tasa_churn_fid)

#Ahora contruimos el targe por cliente
TARGET<-sqldf("select CEDULAENC, fecha, MAX(TARGET) TARGET, MAX(TARGET2) TARGET2  from OBJETIVO_JULIO_AGOSTO group by CEDULAENC ,fecha")
...
OBJETIVO_JULIO_AGOSTO<-merge(x=OBJETIVO_JULIO_AGOSTO, y=fecha_buro, by.x="fecha", by.y="fecha", x.all=TRUE)
OBJETIVO_JULIO_AGOSTO$fecha_buro<-as.numeric(OBJETIVO_JULIO_AGOSTO$fecha_buro)
#table(OBJETIVO_JULIO_AGOSTO$fecha,OBJETIVO_JULIO_AGOSTO$fecha_buro, useNA="ifany")
OBJETIVO_JULIO_AGOSTO$clave_buro<-paste0(OBJETIVO_JULIO_AGOSTO$CEDULAENC,"_", OBJETIVO_JULIO_AGOSTO$fecha_buro)
OBJETIVO_JULIO_AGOSTO$clave<-paste0(OBJETIVO_JULIO_AGOSTO$CEDULAENC,"_", OBJETIVO_JULIO_AGOSTO$fecha)

#Cruce de la poblacion objetivo con la base de datos de buro
#Creamos clave en la población objetivo
#memory.limit(size=65000)

names(OBJETIVO_JULIO_AGOSTO)<-paste0(names(OBJETIVO_JULIO_AGOSTO),"_x")
names(buro)<-paste0(names(buro),"_y")

OBJETIVO_JULIO_AGOSTO_BURO<-merge(
  x=OBJETIVO_JULIO_AGOSTO,
  y=buro,
  by.x="clave_buro_x", by.y="clave_y",all.x=TRUE)

names(OBJETIVO_JULIO_AGOSTO_BURO)<-gsub(pattern="_x", "_objetivo",names(OBJETIVO_JULIO_AGOSTO_BURO))
names(OBJETIVO_JULIO_AGOSTO_BURO)<-gsub(pattern="_y", "_buro",names(OBJETIVO_JULIO_AGOSTO_BURO))

#Grabamos el archivo
save(OBJETIVO_JULIO_AGOSTO_BURO,file="OBJETIVO_JULIO_AGOSTO_BURO.Rdata")

#Cargamos el archivo
load(file="OBJETIVO_JULIO_AGOSTO_BURO.Rdata")

BBDD<-OBJETIVO_JULIO_AGOSTO_BURO
...
save(BBDD2, BBDD3,file = "base_objetivo_fid_Julio_agosto.RData")
