...
modelo_randomForest11<-randomForest(TARGET_objetivo ~., data=smote_training,  ntree=500, nsplit=10)
plot(modelo_randomForest11)
gc()

smote_training2<-smote_training2[ , !(names(smote_training) %in% eliminar)]
modelo_randomForest12<-randomForest(TARGET2_objetivo ~., data=smote_training2, ntree=500, nsplit=10)
plot(modelo_randomForest12)
gc()
save(modelo_randomForest11, modelo_randomForest12, file="modelo_random_smote.Rdata" )

#Modelamiento regresión logística
eliminar<-c("Class") 
smote_training<-smote_training[ , !(names(smote_training) %in% eliminar)]
modelo_logistic21<-glm(TARGET_objetivo ~., family=binomial(link='logit'),data=smote_training, maxit = 100)
...
modelo_logistic22<-glm(TARGET2_objetivo ~., family=binomial(link='logit'),data=smote_training2)
#plot(modelo_logistic22)
#gc()

save(modelo_logistic21, modelo_logistic22, file="modelo_logistic_smote.Rdata" )
