...
tabla_negocio_precision<-sqldf("select prob_testing2, churn_predict, TARGET2_objetivo, count() from BBDD_testing2 group by prob_testing2, churn_predict, TARGET2_objetivo")
write.table(tabla_negocio_precision,file=paste("SALIDA/RF_smote2_tabla_negocio_precision_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 

#???Capa de linealizacion

#funcion para calcular decil
linealizacion <- function(x) {
  quantile(x, probs = seq(0,1, by=0.001),na.rm = TRUE)
}

etiq<-seq(0,1, by=0.001)
capa<-linealizacion(BBDD_training2$churn.predict.prob_training2)
lineal<-cbind(as.data.frame(etiq), as.data.frame(capa))

write.table(BBDD_training2$churn.predict.prob_training2 ,file=paste("SALIDA/training_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 

prueba <- cut(churn.predict.prob_training2, breaks = c(-Inf, lineal$capa), labels = capa$etiq)
barplot(churn.predict.prob_training2)
