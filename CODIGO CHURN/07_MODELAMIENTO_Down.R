...
#save(down_training, down_training2, file="Random_model_down.Rdata")
...
# for(i in trees){
#     aux<-''
#     modelo_randomForest11<-randomForest(TARGET_objetivo ~., data=down_training,  ntree=i)
...
#     print(paste0("auc=", au))
# }
# write.table(list_auc,file=paste("SALIDA/Random_forest_down_iteracion",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
# 
# list_auc_fid<-''
...
# for(i in trees){
#     aux<-''
#     modelo_randomForest12<-randomForest(TARGET2_objetivo ~., data=down_training2,  ntree=i)
...
#write.table(list_auc_fid,file=paste("SALIDA/Random_forest_down_iteracion_fid",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

#Modelamiento: RandomForest
eliminar<-c("Class")
down_training<-down_training[ , !(names(down_training) %in% eliminar)]
modelo_randomForest11<-randomForest(TARGET_objetivo ~., data=down_training,  ntree=200)
#plot(modelo_randomForest11)
gc()

save(down_training,testing ,training, modelo_randomForest11,  file="modelo_random11_down.Rdata" )

varImpPlot(modelo_randomForest11, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
write.table(importance(modelo_randomForest11),file=paste("SALIDA/Random_forest_importance_down_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

plot(modelo_randomForest11$err.rate[,1], log = "y") 
modelo_randomForest11$err.rate[200,1]
...
save(modelo.pred_testing   ,modelo.result_testing   , modelo.F1_testing    , file="Random_forest_down_testing.Rdata")
...
save(modelo.pred_training  ,modelo.result_training  , modelo.F1_training    , file="Random_forest_down_training.Rdata")
...
eliminar<-c("Class")
down_training2<-down_training2[ , !(names(down_training2) %in% eliminar)]
modelo_randomForest12<-randomForest(TARGET2_objetivo ~., data=down_training2, ntree=200)
#plot(modelo_randomForest12)
gc()

save(down_training,testing ,training, modelo_randomForest12,  file="modelo_random12_down.Rdata" )

varImpPlot(modelo_randomForest12, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
write.table(importance(modelo_randomForest12),file=paste("SALIDA/Random_forest_importance_down_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
