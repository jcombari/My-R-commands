...
super_HUELLAS2<-super_HUELLAS

super_HUELLAS2$largo_CEDULA<-str_length(super_HUELLAS2$No__IDENTIFICACION)

freq(as.numeric(super_HUELLAS2$largo_CEDULA), useNA="ifany")

revisar<-sqldf("select * from super_HUELLAS2 where largo_CEDULA<11")
setwd(ruta)
write.xlsx(as.data.frame(names(revisar)),file=paste0("SALIDA/revisar","_" ,format(today, format="%Y%m%d"),".xlsx"),row.names = FALSE, col.names = TRUE)
...
#Analiis rapido de distribuciones
