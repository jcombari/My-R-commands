...
gc()
eliminar<-c("Class")
incluir<-c("TARGET2_objetivo","CARTERA_HIPOTE_ALT_MAX_DE_MORA_buro","CTAS_EMBARGADAS_CTAS_BANCARIAS_buro","CARTERA_RECUPE_CARTERA_HIST_buro","CARTERA_COOPE_ALT_MAX_DE_MORA_buro","min_RESULTADO_SCORE_huellas","ESTADO_CONSULTA_buro","TOTAL_CONSULTAS_ULT_6_MESES_buro","CTAS_CTES_ACT_CTA_BANCA_buro","VLR_MORA_objetivo","PROMCUPOSECTORBANCASIN_POPULAR_buro")
rose_training2<-rose_training2[ , (names(rose_training2) %in% incluir)]
rose_training2<-rose_training2[ , !(names(rose_training2) %in% eliminar)]
#Fitting the Logistic Regression Model:
modelo_logistic_11_full <- glm(TARGET2_objetivo ~.,family=binomial(link="logit"),data=rose_training2)
...
dev.off()
save(modelo_logistic_11, file="RL_rose2.Rdata")
save(modelo_logistic_11, fitted.results.prob, fitted.results,confusionMatrix, pr, auc,file="salida/LR_rose2_metrics_testing2.Rdata")
capture.output(print(modelo_logistic_11, na.print=''), file=paste("SALIDA/LR_rose2_model_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix , na.print=''), file=paste("SALIDA/LR_rose2_confusion_testing2_",format(today, format="%Y%m%d"),".txt", sep=""))
