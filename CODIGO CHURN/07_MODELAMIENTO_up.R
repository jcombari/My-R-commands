...
#Balanceo
load(file="Balanceo_up.Rdata.Rdata")
gc()
...
load(file="modelo_random11_up.Rdata")

#Analisis

varImpPlot(modelo_randomForest11, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
write.table(importance(modelo_randomForest11),file=paste("SALIDA/Random_forest_importance_up_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
