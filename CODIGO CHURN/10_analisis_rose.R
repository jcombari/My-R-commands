...
#Modelo
load(file="RF_rose.Rdata")
print(modelo_randomForest11$confusion)

#importance
png(file =paste("SALIDA/RF_rose_importance_",format(today, format="%Y%m%d"),".png", sep=""), bg = "transparent")
varImpPlot(modelo_randomForest11, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
dev.off()
write.table(importance(modelo_randomForest11),file=paste("SALIDA/RF_rose_importance_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
...
save(modelo_randomForest11, churn.predict.prob, churn.predict.prob,confusionMatrix, pr, auc,file="salida/RF_rose_metrics_testing.Rdata")
...
png(file =paste0("SALIDA/RF_rose_ROC_training_",format(today, format="%Y%m%d"),".png", sep=""), bg = "transparent")
plot(prf_training ,main=paste("ROC Curve for Random Forest Training AUC=", round(auc_training,2) ),col=2,lwd=2)
abline(a=0,b=1,lwd=2,lty=2,col="gray")
dev.off()

save(modelo_randomForest11, churn.predict.prob_training, churn.predict.prob_training,confusionMatrix_training, pr_training, auc_training,file="SALIDA/RF_rose_metrics_training.Rdata")

capture.output(print(modelo_randomForest11, na.print=''), file=paste("SALIDA/RF_rose_confusion_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix , na.print=''), file=paste("SALIDA/RF_rose_confusion_testing_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix_training , na.print=''), file=paste("SALIDA/RF_rose_confusion_training_",format(today, format="%Y%m%d"),".txt", sep=""))
...
write.table(resumen_prob_testing ,file=paste("SALIDA/RF_rose_back_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_saldo_testing,file=paste("SALIDA/RF_rose_back_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_MADURACION_SALDO_testing ,file=paste("SALIDA/RF_rose_back_puntos_corte_mad_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

write.table(resumen_prob_training ,file=paste("SALIDA/RF_rose_back_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_saldo_training,file=paste("SALIDA/RF_rose_back_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_MADURACION_SALDO_training ,file=paste("SALIDA/RF_rose_back_puntos_corte_mad_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

#Tablas para el análisis
write.table(rbind(freq(BBDD_training$prob_training, useNA="ifany"),freq(BBDD_testing$prob_testing, useNA="ifany")),file=paste("SALIDA/RF_rose_back_training_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(rbind(freq(BBDD_training$SAL_CAPITA_training, useNA="ifany"),freq(BBDD_testing$SAL_CAPITA_testing, useNA="ifany")),file=paste("SALIDA/RF_rose_back_training_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(rbind(freq(BBDD_training$MADURACION_SALDO_training, useNA="ifany"),freq(BBDD_testing$MADURACION_SALDO_testing, useNA="ifany")),file=paste("SALIDA/RF_rose_back_training_puntos_corte_maduracion_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

BBDD_trainingB<-sqldf("select * from BBDD_training where TARGET_objetivo=1 ")

write.table(rbind(table(BBDD_training$SAL_CAPITA_training, BBDD_training$prob_training, useNA="ifany"), table(BBDD_trainingB$SAL_CAPITA_training, BBDD_trainingB$prob_training, useNA="ifany")),file=paste("SALIDA/RF_rose_tabla_saldo_prob_training_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_training$MADURACION_SALDO_training, BBDD_training$prob_training, useNA="ifany"),table(BBDD_trainingB$MADURACION_SALDO_training, BBDD_trainingB$prob_training, useNA="ifany") ),file=paste("SALIDA/RF_rose_tabla_maduracion_prob_training_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 

BBDD_testingB<-sqldf("select * from BBDD_testing where TARGET_objetivo=1 ")
write.table(rbind(table(BBDD_testing$SAL_CAPITA_testing, BBDD_testing$prob_testing, useNA="ifany"), table(BBDD_testingB$SAL_CAPITA_testing, BBDD_testingB$prob_testing, useNA="ifany")),file=paste("SALIDA/RF_rose_tabla_saldo_prob_testing_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_testing$MADURACION_SALDO_testing, BBDD_testing$prob_testing, useNA="ifany"),table(BBDD_testingB$MADURACION_SALDO_testing, BBDD_testingB$prob_testing, useNA="ifany") ),file=paste("SALIDA/RF_rose_tabla_maduracion_prob_testing_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
