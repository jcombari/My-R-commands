...
gc()
eliminar<-c("Class")
incluir<-c("TARGET_objetivo","MAX_SAL_CAPITA_objetivo","min_MADURACION_SALDO_objetivo","CTAS_SALDADAS_CTAS_BANCA_buro","VALOR_CUOTAS_SIN_POPULAR_buro","VLR_CUOTA_objetivo","NUMERO_CREDITOS_CODEUDORES_buro","PROMEDIO_CUPO_TDC_SIN_POPULAR_buro","ACIERTA_A_FINANCIERO_buro","mora_max_actual","TOTAL_CONSULTAS_ULT_6_MESES_buro")

up_training<-up_training[ , (names(up_training) %in% incluir)]
up_training<-up_training[ , !(names(up_training) %in% eliminar)]
#Fitting the Logistic Regression Model:
modelo_logistic_11_full <- glm(TARGET_objetivo ~.,family=binomial(link="logit"),data=up_training)
...
dev.off()
save(modelo_logistic_11, file="RL_up.Rdata")
save(modelo_logistic_11, fitted.results.prob, fitted.results,confusionMatrix, pr, auc,file="salida/LR_up_metrics_testing.Rdata")
capture.output(print(modelo_logistic_11, na.print=''), file=paste("SALIDA/LR_up_model_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix , na.print=''), file=paste("SALIDA/LR_up_confusion_testing_",format(today, format="%Y%m%d"),".txt", sep=""))
