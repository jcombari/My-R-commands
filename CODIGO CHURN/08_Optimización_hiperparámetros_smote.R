...
#smote_training<-downSample(x=training, y=training$TARGET_objetivo)
#smote_training2<-downSample(x=training2, y=training2$TARGET2_objetivo)
#save(smote_training, smote_training2, file="balanceo_smote.Rdata")
load(file="balanceo_smote.Rdata")
...
hiperparametro_mtry <-  tuning_rf_mtry(df = smote_training, y = "TARGET_objetivo")

#Ver y analizarla siguiente tabla 
hiperparametro_mtry %>% arrange(oob_err_rate)
grafico_mtry(hiperparametro_mtry)
save(hiperparametro_mtry, file="hiperparametros_smote.Rdata")
#Se debe seleccionar el valor del mtry y reemplazarlo en la función tuning_rf_nodesize
#Hiperparametro nodesize

hiperparametro_nodesize <-  tuning_rf_nodesize(df = smote_training, y = "TARGET_objetivo",
                                               size = c(1:20), mtry=17)

hiperparametro_nodesize %>% arrange(oob_err_rate)

save(hiperparametro_mtry, hiperparametro_nodesize, file="hiperparametros_smote.Rdata")
...
modelo_randomforest <- randomForest(TARGET_objetivo ~ ., data = smote_training, mtry = 17, ntree = 500,
...
save(hiperparametro_mtry, hiperparametro_nodesize, oob_err_rate, file="hiperparametro_RF_smote.Rdata")
...
set.seed(1234)
smote_training$Class

modelo_randomforest <- randomForest(TARGET_objetivo ~ ., data = smote_training, mtry = 10, ntree = 50,
                                    importance = TRUE, nodesize = 1,
                                    norm.votes = TRUE )

save(modelo_randomforest,file="Random_forest_model_smote.Rdata")
...
write.table(importance(modelo_randomforest),file=paste("SALIDA/Random_forest_importance_smote_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
...
save(modelo.pred_testing   ,modelo.result_testing   , modelo.F1_testing    , file="Random_forest_results_smote_testing.Rdata")
...
save(modelo.pred_training  ,modelo.result_training  , modelo.F1_training    , file="Random_forest_results_smote_training.Rdata")
