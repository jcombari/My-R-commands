...
# 
# up_training2<-up_training[ , !(names(up_training) %in% eliminar)]
# trees<-c(20,50,100,200)
# list_auc<-''
# for(i in trees){
#     aux<-''
#     modelo_randomForest12<-randomForest(TARGET2_objetivo ~., data=up_training,  ntree=i)
...
#     print(paste0("auc=", au))
# }
# write.table(list_auc,file=paste("SALIDA/Random_forest_up_iteracion",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
# 
# list_auc_fid<-''
...
# for(i in trees){
#     aux<-''
#     modelo_randomForest12<-randomForest(TARGET22_objetivo ~., data=up_training2,  ntree=i)
...
#write.table(list_auc_fid,file=paste("SALIDA/Random_forest_up_iteracion_fid",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

#Modelamiento: RandomForest
load(file="modelo_random12_up.Rdata")

#Analisis

varImpPlot(modelo_randomForest12, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
write.table(importance(modelo_randomForest12),file=paste("SALIDA/Random_forest_importance_up_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
...
save(modelo.pred_testing   ,modelo.result_testing   , modelo.F1_testing    , file="Random_forest_up_testing2.Rdata")

print("Matriz de confusion")
print(modelo.result_testing$table)

# Evaluation metrics training
training$OBLIGA_MORA_60_CARTERA_ACTUAL_buro<-0
training$OBLIGA_MORA_90_CARTERA_ACTUAL_buro<-0
training$CTAS_EN_COBRADOR_CARTERA_ACT_buro<-0
training$VAL_MORASECTORBANCASIN_POPULAR_buro<-0
training$VAL_CUOTAS_SECTOR_HIP_SIN_POPU_buro<-0
training$VAL_MORA_SECTOR_HIP_SIN_POPU_buro<-0
...
save(modelo.pred_training  ,modelo.result_training  , modelo.F1_training    , file="Random_forest_up_training2.Rdata")
...
write.table(clientes,file=paste("SALIDA/Random_forest_up_tabla_seg_clientes",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(churn,file=paste("SALIDA/Random_forest_up_tabla_seg_churn",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
...
save(BBDD_testing, BBDD_training, file="random_forest11_up.Rdata")
table(BBDD_testing$TARGET2,BBDD_testing$modelo.pred_testing, useNA="ifany")

write.table(BBDD_testing,file=paste("SALIDA/Random_forest_up_BBDD_testing",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(BBDD_training,file=paste("SALIDA/Random_forest_up_BBDD_testing",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

### Modelo con fidelizaciones
eliminar<-c("Class")
up_training2<-up_training2[ , !(names(up_training2) %in% eliminar)]
modelo_randomForest12<-randomForest(TARGET22_objetivo ~., data=up_training2, ntree=200)
#plot(modelo_randomForest12)
gc()

save(up_training,testing ,training, modelo_randomForest12,  file="modelo_random12_up.Rdata" )
...
modelo.result_testing        <- confusionMatrix(data = modelo.pred_testing, testing$TARGET2_objetivo)  
modelo.F1_testing          <- modelo.result_testing  $byClass['F1']
save(modelo.pred_testing   ,modelo.result_testing   , modelo.F1_testing    , file="Random_forest_up_testing.Rdata")
...
save(modelo.pred_training  ,modelo.result_training  , modelo.F1_training    , file="Random_forest_up_training.Rdata")
