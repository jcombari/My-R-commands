...
query=paste0("select * from  BBDD4 where fecha between ",as.character(lista_ini[1]), " and ",as.character(lista_ini[2]))
OBJETIVO_JULIO_AGOSTO<-sqldf(query,verbose=TRUE) 
OBJETIVO_JULIO_AGOSTO$TARGET<-OBJETIVO_JULIO_AGOSTO$MARCA_PREPAGO_RANGO4
freq(OBJETIVO_JULIO_AGOSTO$fecha)
names(OBJETIVO_JULIO_AGOSTO)
freq(OBJETIVO_JULIO_AGOSTO$CUOTA_PAGA2)

#FILTRO: SE DEBE FILTRAR DESEMBOLSOS MAYORES A SEIS MESES.
OBJETIVO_JULIO_AGOSTO<-sqldf("select * from OBJETIVO_JULIO_AGOSTO where CUOTA_PAGA2>6 ")
freq(OBJETIVO_JULIO_AGOSTO$CUOTA_PAGA2)
save(OBJETIVO_JULIO_AGOSTO, file="POBLACION_OBJETIVO_JULIO_AGOSTO.Rdata")
