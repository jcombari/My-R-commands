...
gc()
eliminar<-c("Class")
incluir<-c("TARGET2_objetivo","min_MADURACION_SALDO_objetivo","MAX_CUPO_TDC_SIN_POPULAR_buro","VALOR_CUOTAS_SIN_POPULAR_buro","CTAS_SALDADAS_CTAS_BANCA_buro","ACIERTA_A_FINANCIERO_buro","mora_max_actual","MAX_SAL_CAPITA_objetivo","RANGO_APROXIMADO_EDAD_buro","QUANTO2_buro","CUPO_SIN_POPULAR_buro")
up_training2<-up_training2[ , (names(up_training2) %in% incluir)]
up_training2<-up_training2[ , !(names(up_training2) %in% eliminar)]
#Fitting the Logistic Regression Model:
modelo_logistic_11_full <- glm(TARGET2_objetivo ~.,family=binomial(link="logit"),data=up_training2)
...
dev.off()
save(modelo_logistic_11, file="RL_up2.Rdata")
save(modelo_logistic_11, fitted.results.prob, fitted.results,confusionMatrix, pr, auc,file="salida/LR_up2_metrics_testing2.Rdata")
capture.output(print(modelo_logistic_11, na.print=''), file=paste("SALIDA/LR_up2_model_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix , na.print=''), file=paste("SALIDA/LR_up2_confusion_testing2_",format(today, format="%Y%m%d"),".txt", sep=""))
