...
BBDD_training2<-sqldf("select * from BBDD_training where TARGET_objetivo=1 ")

write.table(rbind(table(BBDD_training$SAL_CAPITA_training, BBDD_training$prob_training, useNA="ifany"), table(BBDD_training2$SAL_CAPITA_training, BBDD_training2$prob_training, useNA="ifany")),file=paste("SALIDA/RF_down_tabla_saldo_prob_training_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_training$MADURACION_SALDO_training, BBDD_training$prob_training, useNA="ifany"),table(BBDD_training2$MADURACION_SALDO_training, BBDD_training2$prob_training, useNA="ifany") ),file=paste("SALIDA/RF_down_tabla_maduracion_prob_training_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 


BBDD_testing2<-sqldf("select * from BBDD_testing where TARGET_objetivo=1 ")
write.table(rbind(table(BBDD_testing$SAL_CAPITA_testing, BBDD_testing$prob_testing, useNA="ifany"), table(BBDD_testing2$SAL_CAPITA_testing, BBDD_testing2$prob_testing, useNA="ifany")),file=paste("SALIDA/RF_down_tabla_saldo_prob_testing_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_testing$MADURACION_SALDO_testing, BBDD_testing$prob_testing, useNA="ifany"),table(BBDD_testing2$MADURACION_SALDO_testing, BBDD_testing2$prob_testing, useNA="ifany") ),file=paste("SALIDA/RF_down_tabla_maduracion_prob_testing_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
