...
load(file="balanceo_up2_extras.Rdata")
#Para lo siguiente se requiere tener la BBDD de testing2

#Modelo
load(file="RF_up2.Rdata")
print(modelo_randomForest12$confusion)

#importance
png(file =paste("SALIDA/RF_up2_importance_",format(today, format="%Y%m%d"),".png", sep=""), bg = "transparent")
varImpPlot(modelo_randomForest12, sort=T, n.var = 10, main ='Top 10 - variables - bosque aleatorio - churn')
dev.off()
write.table(importance(modelo_randomForest12),file=paste("SALIDA/RF_up2_importance_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
...
png(file =paste0("SALIDA/RF_up2_ROC_testing2_",format(today, format="%Y%m%d"),".png", sep=""), bg = "transparent")
plot(prf ,main=paste("ROC Curve for Random Forest testing2 AUC=",  round(auc,2) ) ,col=2,lwd=2)
abline(a=0,b=1,lwd=2,lty=2,col="gray")
dev.off()

save(modelo_randomForest12, churn.predict.prob, churn.predict.prob,confusionMatrix, pr, auc,file="salida/RF_up2_metrics_testing2.Rdata")
...
png(file =paste0("SALIDA/RF_up2_ROC_training2_",format(today, format="%Y%m%d"),".png", sep=""), bg = "transparent")
plot(prf_training2 ,main=paste("ROC Curve for Random Forest training2 AUC=", round(auc_training2,2) ),col=2,lwd=2)
abline(a=0,b=1,lwd=2,lty=2,col="gray")
dev.off()

save(modelo_randomForest12, churn.predict.prob_training2, churn.predict.prob_training2,confusionMatrix_training2, pr_training2, auc_training2,file="SALIDA/RF_up2_metrics_training2.Rdata")

capture.output(print(modelo_randomForest12, na.print=''), file=paste("SALIDA/RF_up2_confusion_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix , na.print=''), file=paste("SALIDA/RF_up2_confusion_testing2_",format(today, format="%Y%m%d"),".txt", sep=""))
capture.output(print(confusionMatrix_training2 , na.print=''), file=paste("SALIDA/RF_up2_confusion_training2_",format(today, format="%Y%m%d"),".txt", sep=""))
...
write.table(resumen_prob_testing2 ,file=paste("SALIDA/RF_up2_back_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_saldo_testing2,file=paste("SALIDA/RF_up2_back_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_MADURACION_SALDO_testing2 ,file=paste("SALIDA/RF_up2_back_puntos_corte_mad_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

write.table(resumen_prob_training2 ,file=paste("SALIDA/RF_up2_back_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_saldo_training2,file=paste("SALIDA/RF_up2_back_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(resumen_MADURACION_SALDO_training2 ,file=paste("SALIDA/RF_up2_back_puntos_corte_mad_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

#Tablas para el análisis
write.table(rbind(freq(BBDD_training2$prob_training2, useNA="ifany"),freq(BBDD_testing2$prob_testing2, useNA="ifany")),file=paste("SALIDA/RF_up2_back_training2_puntos_corte_prob_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(rbind(freq(BBDD_training2$SAL_CAPITA_training2, useNA="ifany"),freq(BBDD_testing2$SAL_CAPITA_testing2, useNA="ifany")),file=paste("SALIDA/RF_up2_back_training2_puntos_corte_saldo_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)
write.table(rbind(freq(BBDD_training2$MADURACION_SALDO_training2, useNA="ifany"),freq(BBDD_testing2$MADURACION_SALDO_testing2, useNA="ifany")),file=paste("SALIDA/RF_up2_back_training2_puntos_corte_maduracion_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T)

BBDD_training2B<-sqldf("select * from BBDD_training2 where TARGET2_objetivo=1 ")

write.table(rbind(table(BBDD_training2$SAL_CAPITA_training2, BBDD_training2$prob_training2, useNA="ifany"), table(BBDD_training2B$SAL_CAPITA_training2, BBDD_training2B$prob_training2, useNA="ifany")),file=paste("SALIDA/RF_up2_tabla_saldo_prob_training2_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_training2$MADURACION_SALDO_training2, BBDD_training2$prob_training2, useNA="ifany"),table(BBDD_training2B$MADURACION_SALDO_training2, BBDD_training2B$prob_training2, useNA="ifany") ),file=paste("SALIDA/RF_up2_tabla_maduracion_prob_training2_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 

BBDD_testing2B<-sqldf("select * from BBDD_testing2 where TARGET2_objetivo=1 ")
write.table(rbind(table(BBDD_testing2$SAL_CAPITA_testing2, BBDD_testing2$prob_testing2, useNA="ifany"), table(BBDD_testing2B$SAL_CAPITA_testing2, BBDD_testing2B$prob_testing2, useNA="ifany")),file=paste("SALIDA/RF_up2_tabla_saldo_prob_testing2_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
write.table(rbind(table(BBDD_testing2$MADURACION_SALDO_testing2, BBDD_testing2$prob_testing2, useNA="ifany"),table(BBDD_testing2B$MADURACION_SALDO_testing2, BBDD_testing2B$prob_testing2, useNA="ifany") ),file=paste("SALIDA/RF_up2_tabla_maduracion_prob_testing2_",format(today, format="%Y%m%d"),".txt", sep=""),sep=";",row.names = T) 
