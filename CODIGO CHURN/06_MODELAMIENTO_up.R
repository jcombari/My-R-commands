...
modelo_randomForest11<-randomForest(TARGET_objetivo ~., data=up_training,  ntree=50)
plot(modelo_randomForest11)
gc()


save(up_training,testing ,training, modelo_randomForest11,  file="modelo_random11_up.Rdata" )

#plot(modelo_randomForest11$err.rate, log = "y") 


up_training2<-up_training2[ , !(names(up_training2) %in% eliminar)]
modelo_randomForest12<-randomForest(TARGET2_objetivo ~., data=up_training2, ntree=50)
plot(modelo_randomForest12)
gc()
save(up_training2, testing2, training2, modelo_randomForest12,  file="modelo_random12_up.Rdata" )
...
eliminar<-c("Class") 
up_training<-up_training[ , !(names(up_training) %in% eliminar)]
modelo_logistic21<-glm(TARGET_objetivo ~., family=binomial(link='logit'),data=up_training, maxit = 100)
...
up_training2<-up_training2[ , !(names(up_training2) %in% eliminar)]
modelo_logistic22<-glm(TARGET2_objetivo ~., family=binomial(link='logit'),data=up_training2)
#plot(modelo_logistic22)
#gc()

save(modelo_logistic21, modelo_logistic22, file="modelo_logistic_up.Rdata" )
